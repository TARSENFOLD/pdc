...
test.describe('Simulação Tipo 3 (Alta Fidelidade)', () => {
  test('aluno can play a high-fidelity simulation', async ({ alunoPage }) => {
    ...
    // 4. Verificar se o Tipo3Player renderiza
    await expect(alunoPage.locator('text=Concluir Simulação')).toBeVisible({ timeout: 10_000 });
    ...
    // 5. Interagir com o shell
    await alunoPage.click('text=Tomar Decisão');
    ...
    // 6. Concluir
    await alunoPage.click('text=Concluir Simulação');
