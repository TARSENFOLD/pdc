...
function statusFromError(err: unknown): VideoErrorStatus {
  ...
  if (
    ...
    status === 409 || status === 413 || status === 415 || status === 501 || status === 503
  ) {
    return status;
  }
  return 502;
...
async function readBodyWithLimit(
  ...
  let output = new Uint8Array(Math.min(1024 * 1024, maxBytes));
  let totalBytes = 0;
  ...
  try {
    ...
    while (!chunk.done) {
      ...
      if (totalBytes > output.byteLength) {
        const nextCapacity = Math.min(
          maxBytes,
          Math.max(totalBytes, Math.max(output.byteLength * 2, 1)),
        );
        const expanded = new Uint8Array(nextCapacity);
        expanded.set(output);
        output = expanded;
      }
      output.set(chunk.value, totalBytes - chunk.value.byteLength);
...
videoRoutes.put('/:id/content', verifyJwt, async (c) => {
  try {
    ...
    const authorization = await videoService.authorizeContentUpload(videoId, mimeType, c.get('user'));
