...
export function CourseWelcomePage(): React.JSX.Element {
  ...
  const firstIncomplete = allItems.find(
    (item) => !progresso.some((entry) => entry.itemId === routeId(item.id) && entry.concluido)
  );
    ...
    () => ({
      ...
      progress: (
        <div className="flex min-w-44 items-center gap-3">
          <div className="bg-border h-1.5 min-w-24 flex-1 overflow-hidden rounded-full">
            <div className="bg-accent h-full" style={{ width: `${String(progressPercent)}%` }} />
          </div>
      ...
      actions: (
        <button
          ...
          className="text-ink-secondary hover:bg-recessed flex min-h-11 min-w-11 items-center justify-center rounded-lg lg:hidden"
  ...
  if (progressoQuery.isError && isEnrollmentRequiredError(progressoQuery.error)) {
    return (
      <p className="text-error py-12 text-center">Inscreve-te no curso para aceder ao conteúdo.</p>
    );
  }
    ...
    >
      <main className="min-w-0 flex-1 overflow-y-auto">
        <div className="mx-auto flex min-h-full w-full max-w-6xl flex-col px-4 py-6 md:px-8 md:py-10">
          <section className="border-border bg-elevated grid flex-1 overflow-hidden rounded-xl border shadow-sm lg:min-h-[560px] lg:grid-cols-[1.05fr_0.95fr]">
            <div className="flex flex-col justify-between bg-[#2f4d91] px-7 py-10 text-white sm:px-10 lg:px-14 lg:py-14">
              <div>
                ...
                <h1 className="font-display mt-6 max-w-xl text-4xl leading-tight sm:text-5xl lg:text-6xl">
                  {curso.titulo}
                </h1>
              ...
              <div className="mt-10">
                <div className="mb-6 flex flex-wrap gap-x-6 gap-y-3 text-sm text-white/75">
                  <span className="inline-flex items-center gap-2">
                    ...
                    {(curso.modulos?.length ?? 0) === 1 ? 'módulo' : 'módulos'}
                  </span>
                  <span className="inline-flex items-center gap-2">
                    <BookOpen className="h-4 w-4" />
                    {allItems.length} {allItems.length === 1 ? 'aula' : 'aulas'}
                  </span>
                  <span className="inline-flex items-center gap-2">
            ...
            <div className="bg-recessed relative min-h-72 lg:min-h-full">
              {curso.capaUrl ? (
                <img
                  src={curso.capaUrl}
                ...
                <div className="from-accent/20 via-recessed absolute inset-0 flex flex-col items-center justify-center gap-4 bg-gradient-to-br to-[#2f4d91]/20 px-8 text-center">
                  <span className="bg-elevated text-accent flex h-20 w-20 items-center justify-center rounded-full shadow-lg">
                    <BookOpen className="h-9 w-9" />
                  </span>
                  <p className="font-display text-ink-primary max-w-xs text-2xl">
                    A tua aprendizagem começa aqui.
                  </p>
                </div>
              )}
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-6 pt-20 text-white">
                ...
                <p className="mt-1 text-lg font-semibold">
                  {startItem?.titulo ?? 'Currículo em preparação'}
                </p>
              </div>
            </div>
          </section>
          ...
          <div className="border-border mt-5 flex items-center justify-between border-t pt-5">
            <p className="text-ink-secondary hidden text-sm sm:block">
              Explora os módulos na coluna lateral ou segue pela ordem sugerida.
            </p>
            <Button
              type="button"
              variant="secondary"
              ...
              className="ml-auto"
            >
              {completedCount > 0 ? 'Continuar curso' : 'Ir para a primeira aula'}
