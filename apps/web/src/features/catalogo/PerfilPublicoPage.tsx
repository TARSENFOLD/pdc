  ...
  return (
    <div className="min-h-screen bg-background px-4 py-20 sm:px-8">
      ...
      <div className="mx-auto max-w-4xl">
        <Link to="/explorar" className="group inline-flex items-center gap-2 text-sm font-bold text-text-muted hover:text-amber transition-colors mb-10">
          <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> 
          Voltar para Explorar
        </Link>

        <header className="relative rounded-3xl border border-border bg-surface p-8 sm:p-12 overflow-hidden shadow-sm">
          <div className="absolute right-0 top-0 h-40 w-40 bg-amber/5 blur-3xl rounded-full -translate-y-1/2 translate-x-1/2" />
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8 relative z-10">
            ...
            <div className="flex-1 text-center sm:text-left">
              <div className={`inline-flex rounded-full border px-3 py-0.5 text-[10px] font-black uppercase tracking-widest mb-3 ${roleColor[perfil.role] || 'text-text-muted border-border bg-white/5'}`}>
              ...
              <div className="mt-6 flex flex-wrap justify-center sm:justify-start gap-4">
                ...
                <Button variant="secondary" className="rounded-xl px-8 font-bold text-text-primary">Seguir Trajetória</Button>
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {perfil.bio && (
              <section className="rounded-3xl border border-border bg-surface p-8 shadow-sm">
                <h2 className="text-xl font-bold text-text-primary mb-4 flex items-center gap-2">
                  <span className="h-1.5 w-1.5 rounded-full bg-amber" /> Bio
                </h2>
                <p className="text-sm text-text-secondary leading-relaxed whitespace-pre-wrap">{perfil.bio}</p>
              </section>
            )}

            <section className="rounded-3xl border border-border bg-surface p-8 shadow-sm">
              <h2 className="text-xl font-bold text-text-primary mb-6 flex items-center gap-2">
                <span className="h-1.5 w-1.5 rounded-full bg-amber" /> Projetos Públicos
              </h2>
              <div className="flex flex-col items-center justify-center py-10 text-center rounded-2xl border border-dashed border-border bg-surface-alt">
                <Lock size={32} className="text-text-muted mb-4" />
                <h3 className="text-sm font-bold text-text-primary">Conteúdo Restrito</h3>
                <p className="mt-2 text-xs text-text-secondary max-w-xs mx-auto">Os projetos deste utilizador são visíveis apenas para membros registados da comunidade PDC.</p>
                <Link to="/criar-conta" className="mt-6 text-xs font-black uppercase tracking-widest text-amber hover:underline">Criar conta gratuita</Link>
              </div>
            </section>
          </div>
          ...
          <aside className="space-y-6">
            <Card className="p-6 border-border bg-surface shadow-sm rounded-2xl">
              <h3 className="text-sm font-black uppercase tracking-widest text-text-muted mb-4">Informação</h3>
            ...
            <Card className="p-6 border-border bg-surface shadow-sm rounded-2xl">
              ...
              {perfil.competencias && perfil.competencias.length > 0 ? (
                ...
                <p className="text-xs text-text-muted italic">Nenhuma competência listada.</p>
              )}
            </Card>

            <Card className="p-6 border-border bg-amber/5 shadow-sm rounded-2xl border-amber/20">
              <h3 className="text-sm font-black uppercase tracking-widest text-amber mb-4 flex items-center justify-between">
                Mérito <Award size={14} />
              </h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-text-secondary">Reputação</span>
                  <span className="font-bold text-text-primary">78/100</span>
                </div>
                <div className="h-1.5 w-full bg-black/20 rounded-full overflow-hidden">
                  <div className="h-full bg-amber rounded-full" style={{ width: '78%' }} />
                </div>
                <p className="text-[10px] text-text-muted leading-relaxed mt-2 italic">A reputação é calculada com base na telemetria comportamental e validações do comité.</p>
              </div>
            </Card>
          </aside>
        </div>
