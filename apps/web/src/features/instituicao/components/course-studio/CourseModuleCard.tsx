...
export function CourseModuleCard({
  ...
  const [activeItemIndex, setActiveItemIndex] = useState<number | null>(
    moduleIndex === 0 || initiallyOpen ? 0 : null
  );
  ...
  useEffect(() => {
    ...
    document
      .getElementById(`course-module-${String(moduleIndex)}-item-${String(newItemIndex)}`)
  ...
  const changeItemType = (itemIndex: number, type: CourseItemType) => {
    ...
    if (
      type === 'video' &&
      module.itens.some((item, index) => index !== itemIndex && item.tipo === 'video')
    )
      return;
    const nextItems = module.itens.map((item, index) =>
      index === itemIndex ? { ...item, tipo: type } : item
    );
  ...
  const dropItem = (event: DragEvent<HTMLElement>, targetIndex: number) => {
    ...
    const [sourceModule, sourceIndex] = event.dataTransfer
      .getData('application/x-pdc-course-item')
      .split(':')
      .map(Number);
    if (sourceModule !== moduleIndex || sourceIndex === undefined || !Number.isInteger(sourceIndex))
      return;
  ...
  return (
    <article
      ...
      className="border-border bg-elevated overflow-hidden rounded-xl border shadow-[var(--elevation-1)]"
      onDragOver={(event) => {
        event.preventDefault();
      }}
      onDrop={onDrop}
    >
      <header className="border-border bg-canvas flex min-h-[76px] items-center gap-2 border-b px-3 sm:px-5">
        <button
          type="button"
          draggable
        ...
        <button
          ...
          aria-expanded={open}
        >
          <span className="border-accent/30 bg-accent/10 text-accent flex h-10 w-10 shrink-0 items-center justify-center rounded-full border text-sm font-bold">
            {moduleIndex + 1}
          </span>
          <span className="min-w-0">
            <span className="text-ink-primary block truncate text-base font-bold">
              {module.titulo}
            </span>
            <span className="text-ink-tertiary mt-0.5 block text-xs">
              {module.itens.length} {module.itens.length === 1 ? 'aula' : 'aulas'}
            </span>
          </span>
        </button>
        <button
          ...
          aria-label={open ? 'Recolher módulo' : 'Expandir módulo'}
        >
      ...
      {open ? (
        <div className="space-y-6 p-4 sm:p-6">
          <div className="border-border grid gap-4 border-b pb-6 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          ...
          <div>
            <div>
              <h4 className="text-ink-primary flex items-center gap-2 text-sm font-bold">
                <Layers3 size={16} className="text-accent" /> Aulas do módulo
              </h4>
              <p className="text-ink-tertiary mt-1 max-w-2xl text-xs leading-5">
                Cada aula será uma página independente no consumo. Abre apenas a aula que queres
                editar.
              </p>
            </div>
          </div>
            ...
            {itemsArray.fields.map((field, itemIndex) => (
              <CourseLessonCard
                ...
                onOpenChange={(nextOpen) => {
                  setActiveItemIndex(nextOpen ? itemIndex : null);
                }}
                ...
                videoTypeAvailable={
                  !module.itens.some(
                    (candidate, candidateIndex) =>
                      candidateIndex !== itemIndex && candidate.tipo === 'video'
                  )
                }
                onMove={(direction) => {
                ...
                onDragStart={(event) => {
                  event.dataTransfer.setData(
                    'application/x-pdc-course-item',
                    `${String(moduleIndex)}:${String(itemIndex)}`
                  );
                }}
                onDrop={(event) => {
                  dropItem(event, itemIndex);
                }}
              />
            ))}
          </div>
          ...
          <div className="border-border bg-recessed/30 rounded-lg border border-dashed p-4 text-center">
            ...
            <p className="text-ink-tertiary mt-2 text-xs">
              A nova aula será aberta para edição. O vídeo fica no início do módulo; os restantes
              formatos são adicionados no fim.
            </p>
          </div>

          {itemsArray.fields.length === 1 ? (
            <p className="text-ink-tertiary text-xs">Cada módulo precisa de pelo menos uma aula.</p>
          ) : null}
        </div>
      ) : null}
