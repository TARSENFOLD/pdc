...
export function ProjetoPedidosPage(): React.JSX.Element {
  ...
  const decision = useMutation({
    mutationFn: ({ perfilId, action }: { perfilId: string; action: 'aprovar' | 'rejeitar' | 'remover' }) =>
      projetosApi.gerirACL(id ?? '', perfilId, action),
    onSuccess: () => {
  ...
  const entries = project.acessoCoreACL ?? [];
  const pendingCount = entries.filter((entry) => entry.estado === 'pendente').length;
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      <section className="py-6" aria-label="Pedidos de acesso ao núcleo">
        <ProjetoAccessList
          entries={entries}
          pendingPerfilId={decision.isPending ? decision.variables.perfilId : undefined}
          onDecision={(perfilId, action) => { decision.mutate({ perfilId, action }); }}
        />
      </section>
