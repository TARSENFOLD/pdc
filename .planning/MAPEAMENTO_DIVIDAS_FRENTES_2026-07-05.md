...
| **DT-C5** | Schema `Programa` incompleto vs spec 04 §3.4 | `infra/strapi/src/api/programa/.../schema.json` | Faltavam `proposito`, `metodologia`, `cronograma`, `responsavel`, `regrasMatricula`, `precoPolicy`, `criadorTipo` e relações | ✅ **Resolvido no schema**; validar UI/BFF completa em próxima iteração de Programas |
| **DT-C6** | Schema `Projeto` sem "Pitch Seguro" | `infra/strapi/src/api/projeto/.../schema.json`; BFF/UI projeto | Faltavam `abstract`/`core`, `modos`, `pedidosAcesso`, `selo` | ✅ **Resolvido E2E** — collection `projeto-acesso-pedido`, BFF canónico `/pedidos-acesso`, UI de gestão e eventos G15 |
