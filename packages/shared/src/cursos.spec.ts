...
describe('CriarCursoPayloadSchema — composição de mídia', () => {
  it('orienta em português quando a identidade do curso está incompleta', () => {
    const result = CriarCursoPayloadSchema.safeParse({
      ...validCourse(),
      titulo: '',
      descricao: '',
    });
      ...
      expect(result.error.issues).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            path: ['titulo'],
            message: 'Adiciona um título com pelo menos 3 caracteres.',
          }),
          expect.objectContaining({
            path: ['descricao'],
            message: 'Escreve uma descrição com pelo menos 10 caracteres.',
          }),
        ])
      );
    }
  ...
  it('aceita módulo vazio no payload de rascunho, mas não o considera pronto', () => {
    ...
    expect(readiness.byStep.curriculum.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ path: 'modulos.0.itens' })])
    );
  });
  ...
  it('rejeita moeda numérica mesmo quando tem três caracteres', () => {
    ...
    expect(readiness.issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          path: 'moeda',
          message: 'Indica uma moeda válida com 3 letras.',
        }),
      ])
    );
  });
  ...
  it('rejeita mais de um vídeo no mesmo módulo', () => {
    ...
    if (!result.success)
      expect(
        result.error.issues.some((issue) => issue.message.includes('no máximo um vídeo'))
      ).toBe(true);
  });
  ...
  it('rejeita vídeo fora da primeira posição', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success)
      expect(result.error.issues.some((issue) => issue.message.includes('primeiro conteúdo'))).toBe(
        true
      );
  });
  ...
  it('rejeita posições duplicadas mesmo quando o módulo não tem vídeo', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success)
      expect(result.error.issues.some((issue) => issue.message.includes('posição única'))).toBe(
        true
      );
  });
  ...
  it('permite guardar mídia HTTP em rascunho e deixa a prontidão bloquear publicação insegura', () => {
    ...
    expect(
      avaliarProntidaoCurso({ ...persistedExternal, capaUrl: 'https://cdn.example.com/capa.webp' })
        .ready
    ).toBe(false);
    expect(
      avaliarProntidaoCurso({ ...persistedLocal, capaUrl: 'https://cdn.example.com/capa.webp' })
        .ready
    ).toBe(false);
    expect(
      avaliarProntidaoCurso(
...
describe('AtualizarCursoPayloadSchema', () => {
  it('aceita alterações parciais sem currículo', () => {
    expect(AtualizarCursoPayloadSchema.safeParse({ titulo: 'Novo título' }).success).toBe(true);
  });

  it('recusa estado editorial na atualização geral', () => {
    expect(AtualizarCursoPayloadSchema.safeParse({ estado: 'published' }).success).toBe(false);
  });

  it('recusa módulos com posições duplicadas', () => {
    const course = validCourse();
    course.modulos.push({
      titulo: 'Segundo módulo',
      ordem: 1,
    ...
    const result = AtualizarCursoPayloadSchema.safeParse({ modulos: course.modulos });
...
describe('avaliarProntidaoCurso', () => {
  ...
  it('bloqueia curso vazio e identifica cada etapa incompleta', () => {
    ...
    expect(result.byStep.info.issues.map((issue) => issue.path)).toEqual([
      'titulo',
      'descricao',
      'capaUrl',
    ]);
    expect(result.byStep.curriculum.issues.map((issue) => issue.path)).toEqual([
      'modulos.0.itens.0.conteudo',
    ]);
  ...
  it('bloqueia a submissão de um módulo sem aulas', () => {
    ...
    expect(result.ready).toBe(false);
    expect(result.byStep.curriculum.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ path: 'modulos.0.itens' })])
    );
  });
  ...
  it('bloqueia mais de um vídeo e vídeo fora da primeira posição', () => {
          ...
          itens: [
            {
              ...
              tipo: 'texto',
              conteudo: 'Conteúdo completo da leitura.',
              ordem: 1,
            },
    ...
    expect(multipleVideos.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ path: 'modulos.0.itens' })])
    );
    expect(lateVideo.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ path: 'modulos.0.itens.1' })])
    );
  });
          ...
          itens: [
            {
              titulo: 'Documento um',
              tipo: 'pdf',
              url: 'https://cdn.example.com/um.pdf',
              ordem: 1,
            },
  ...
  it('bloqueia galeria HTTP externa na prontidão sem impedir o rascunho', () => {
    ...
    expect(result.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ path: 'modulos.0.itens.0.imagens.0.url' })])
    );
  });
  ...
  it('reporta uma galeria legada malformada sem lançar erro', () => {
    ...
    expect(result.byStep.curriculum.complete).toBe(false);
    expect(result.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ path: 'modulos.0.itens.0.imagens.0.alt' })])
    );
  });
  ...
  it.each([undefined, 'audio'])('rejeita formato de aula ausente ou desconhecido: %s', (tipo) => {
    const result = avaliarProntidaoCurso({
      ...
      modulos: [
        {
          titulo: 'Formato',
          itens: [{ titulo: 'Aula inválida', tipo, conteudo: 'Conteúdo suficiente.', ordem: 1 }],
        },
      ],
    });

    expect(result.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ path: 'modulos.0.itens.0.tipo' })])
    );
  });
});
