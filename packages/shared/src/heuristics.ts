...
export const analyzeHesitation = (hesitationTimeMs: number): HeuristicResult => {
  if (hesitationTimeMs < 1000) return {
    level: 'EXCELENTE',
    insight: 'Decisões ultra-rápidas sem dúvida.',
    action: 'Aumentar a complexidade para não gerar tédio.',
    score: 9.9
  };
  if (hesitationTimeMs < 5000) return {
    level: 'ESTAVEL',
    insight: 'Ponderação analítica normal antes de agir.',
    action: 'Manter a carga de raciocínio atual.',
    score: 7.5
  };
  return {
    level: 'VULNERAVEL',
    insight: 'Períodos de paralisia antes da acção. Possível confusão na interface ou tarefa.',
    action: 'Investigar se a tarefa está perfeitamente clara.',
    score: 4.0
  };
};
